...
impl Header {
    ...
    pub fn genesis() -> Self {
        Self { parent: 0, height: 0, extrinsics_root: 0, state: 0, consensus_digest: 0 }
    }
...
impl Block {
    ...
    pub fn genesis() -> Self {
        Self { header: Header::genesis(), body: Vec::new() }
    }
    ...
    pub fn child(&self, extrinsics: Vec<u64>) -> Self {
        Self { header: self.header.child(hash(&extrinsics), 0), body: extrinsics }
    }
    ...
    pub fn verify_sub_chain(&self, chain: &[Block]) -> bool {
        ...
        let next = &chain[0];
        if !self.header.verify_child(&next.header){
            return false;
        }
        if next.header.extrinsics_root != hash(&next.body){
            return false;
        }
        next.verify_sub_chain(&chain[1..])
    }
...
fn build_invalid_child_block_with_valid_header(parent: &Header) -> Block {
    ...
    Block { header: parent.child(hash(&extrinsics), 9), body: Vec::new() }
}
